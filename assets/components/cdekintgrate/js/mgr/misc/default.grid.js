cdekIntgrate.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        url: cdekIntgrate.config['connector_url'],
        baseParams: {},
        cls: config['cls'] || 'main-wrapper cdekintgrate-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('cdekintgrate-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('cdekintgrate-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('cdekintgrate-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('cdekintgrate-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    cdekIntgrate.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(cdekIntgrate.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (width) {
        return {
            xtype: 'cdekintgrate-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = cdekIntgrate.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('cdekintgrate-grid-default', cdekIntgrate.grid.Default);